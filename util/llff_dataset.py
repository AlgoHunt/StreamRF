...
class LLFFDataset(DatasetBase):
    ...
    def _load_images(self):
        ...
        for idx in tqdm(range(len(self.imgs)), disable=not self.verbose):
        ...
        if not self.is_train_split:
            render_c2w = []
            for idx in tqdm(range(len(self.sfm.render_poses)), disable= not self.verbose):
        ...
        # Padded bounds
        radx = 1 + 2 * self.sfm.offset / self.gt.shape[2]
        rady = 1 + 2 * self.sfm.offset / self.gt.shape[1]
        radz = 1.0
