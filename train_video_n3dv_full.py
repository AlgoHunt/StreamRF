...
os.makedirs(os.path.join(args.train_dir, 'grid_delta_full'), exist_ok=True)
os.makedirs(os.path.join(args.train_dir, 'grid_delta_full_z'), exist_ok=True)
os.makedirs(os.path.join(args.train_dir, 'test_images_full'), exist_ok=True)
os.makedirs(os.path.join(args.train_dir, 'test_images_depth_full'), exist_ok=True)

logfolder = args.train_dir
if os.path.exists(f'{logfolder}/log_full.log'):
    os.remove(f'{logfolder}/log_full.log')
logger.add(f'{logfolder}/log_full.log' , format="{level} {message}" , level='DEBUG' if args.debug else 'INFO')

summary_writer = SummaryWriter(args.train_dir)

with open(path.join(args.train_dir, 'args_full.json'), 'w') as f:
    json.dump(args.__dict__, f, indent=2)
    # Changed name to prevent errors
    shutil.copyfile(__file__, path.join(args.train_dir, 'train_full_frozen.py'))
...
def compress_saving(grid_pre, grid_next, grid_holder, save_delta=False,saving_name=None):
    ...
    if save_delta:
        ...
        save_path = os.path.join(args.train_dir,'grid_delta_full',f'{saving_name}.pth')
    ...
    if save_delta:
        ...
        save_path = os.path.join(args.train_dir,'grid_delta_full_z',f'{saving_name}.npz')
...
def get_sample_table(curr_reso,reso):
...
if args.dilate_rate_after > 0:
    ...
    sparsify_voxel_grid(grid,dilate=args.dilate_rate_after,weight_thres=0.01)
...
def load_delta(grid_pre, delta_path):
    ...
    return  change_mask, minus_area, mask_next, add_area


grid_pilot = svox2.SparseGrid.load(os.path.join(args.train_dir, "ckpt_pilot_0.npz"), device=device)
config_util.setup_render_opts(grid_pilot.opt, args)
sample_table = get_sample_table(curr_reso=grid_pilot.links.shape, reso=grid.links.shape)

def resample_mask(mask, shape):
    mask = mask.float()
    mask = F.interpolate(mask.unsqueeze(0).unsqueeze(0),(shape[0],shape[1],shape[2]), mode='trilinear')
    mask = (mask>0)[0,0,:,:,:]
    return mask
...
def finetune_one_frame(frame_idx, global_step_base, dsets):
    ...
    with torch.no_grad():
        ...
        minus_mask = resample_mask(minus_mask, grid.links.shape)
      

        # delete_area(grid, minus_mask.view(grid.links.shape))

        if args.dilate_rate_before > 0:
            dilated_voxel_grid(args.dilate_rate_before)
        

        sample_xyz = sample_table[add_mask.view(-1) * (grid.links>=0).view(-1),:]
    
        add_mask = add_mask[grid.links>=0]
        add_mask = add_mask.view(-1)

        
        minus_mask = minus_mask[grid.links>=0]
        minus_mask = minus_mask.view(-1)
        student_mask =  _C.dilate( _C.dilate(~_C.dilate(~student_mask)))
        student_mask = student_mask[grid.links>=0]
        student_mask = student_mask.view(-1)
        
        

        sample_vals_density, sample_vals_sh = grid_pilot.sample(
                        sample_xyz,
                        grid_coords=True,
                        want_colors=True
                    )
        new_sh_data = grid.sh_data 
        new_density_data = grid.density_data 
        new_sh_data[add_mask] = sample_vals_sh
        new_density_data[add_mask] = sample_vals_density

        minus_sum = (minus_mask >0).sum()
        new_sh_data[minus_mask] = torch.zeros(minus_sum,27,dtype=new_sh_data.dtype,device=device)
        new_density_data[minus_mask] = torch.zeros(minus_sum,1,dtype=new_sh_data.dtype,device=device)

        grid.sh_data = nn.Parameter(new_sh_data) 
        grid.density_data = nn.Parameter(new_density_data)

    if args.apply_narrow_band:
        grad_mask = narrow_band[grid.links>=0]
        grad_mask = grad_mask.view(-1)
        grad_mask = grad_mask & student_mask
    else:
        grad_mask = student_mask
    ...
    def render_img():
        ...
        for img_id in tqdm(range(0, n_images, img_eval_interval)):
            ...
            if args.render_all:
                im_path = os.path.join(train_dir, 'test_images_full', f'{frame_idx:04d}_{img_id:02d}.png' )
                depth_path = os.path.join(train_dir, 'test_images_depth_full', f'{frame_idx:04d}_{img_id:02d}.png' )
            else:
                im_path = os.path.join(train_dir, 'test_images_full', f'{frame_idx:04d}.png' )
...
if train_frame_num:
    ...
    vid_path = os.path.join(args.train_dir, tag+'_full.mp4')
